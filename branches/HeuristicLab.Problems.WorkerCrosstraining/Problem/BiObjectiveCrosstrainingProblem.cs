...
namespace HeuristicLab.Problems.WorkerCrosstraining {
  [StorableType("EAD7D63B-8712-4C3B-AC94-54515A8323B9")]
  [Item("Bi-objective Worker Cross Training Problem", "")]
  [Creatable(CreatableAttribute.Categories.Problems)]
  public sealed class BiObjectiveCrosstrainingProblem : MultiObjectiveBasicProblem<QualificationEncoding> {
    ...
    [StorableConstructor]
    private BiObjectiveCrosstrainingProblem(StorableConstructorFlag _) : base(_) { }
    private BiObjectiveCrosstrainingProblem(BiObjectiveCrosstrainingProblem original, Cloner cloner)
    ...
    public BiObjectiveCrosstrainingProblem() : base() {
      ...
      Parameters.Add(workersParameter = new FixedValueParameter<IntValue>(WorkersParameterName, "The number of workers.", new IntValue(46)));
      ...
      Parameters.Add(lineChangeFactorParameter = new FixedValueParameter<DoubleValue>(LineChangeFactorParameterName, "Factor of the CR that is applied when changing between lines.", new DoubleValue(2.0)));
      Parameters.Add(dispatchStrategyParameter = new FixedValueParameter<EnumValue<DispatchStrategy>>(DispatchStrategyParameterName, "The dispatching strategy.", new EnumValue<DispatchStrategy>(DispatchStrategy.FirstComeFirstServe)));
      ...
      Parameters.Add(observationTimeParameter = new FixedValueParameter<DoubleValue>(ObservationTimeParameterName, "The maximum observed simulation time.", new DoubleValue(3600.0)));
      Parameters.Add(warmupTimeParameter = new FixedValueParameter<DoubleValue>(WarmupTimeParameterName, "The initial simulation time that does not count towards the performance.", new DoubleValue(600.0)));
    ...
    public override IDeepCloneable Clone(Cloner cloner) {
      return new BiObjectiveCrosstrainingProblem(this, cloner);
    }
    ...
    public override double[] Evaluate(Individual individual, IRandom random) {
      var vector = individual.Qualification(Encoding.Name).ToBoolArray();
    ...
    public override void Analyze(Individual[] individuals, double[][] qualities, ResultCollection results, IRandom random) {
      ...
      bestFront.ColumnNames = new[] { Objective.ToString(), "TQ" };
